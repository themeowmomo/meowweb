  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        {totalItems > 0 ? (
          <div className="fixed bottom-6 left-4 right-4 z-[60] animate-in fade-in slide-in-from-bottom-6 duration-500">
            <Button className="w-full h-16 bg-foreground text-white rounded-2xl shadow-2xl flex items-center justify-between px-6 group hover:bg-foreground/90 transition-all border-none">
              <div className="flex items-center gap-4">
                <div className="bg-primary p-2 rounded-xl text-white">
                  <ShoppingCart className="w-5 h-5" />
                </div>
                <div className="text-left">
                  <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Order Subtotal</p>
                  <p className="text-lg font-black tracking-tight">Rs.{totalPrice} <span className="text-[10px] opacity-40 ml-1">({totalItems} Items)</span></p>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-xl group-hover:bg-white/20 transition-colors">
                <span className="text-[10px] font-black uppercase tracking-widest">Open Cart</span>
                <ArrowRight className="w-4 h-4" />
              </div>
            </Button>
          </div>
        ) : (
          <Button variant="outline" size="icon" className="relative border-primary/20 bg-white hover:bg-secondary h-12 w-12 rounded-xl shadow-sm">
            <ShoppingCart className="w-5 h-5 text-primary" />
          </Button>
        )}
      </SheetTrigger>
      <SheetContent className="w-full sm:max-w-md flex flex-col p-0 border-l-0 sm:border-l rounded-t-[2rem] sm:rounded-l-[2rem] overflow-hidden">
        <div className="px-6 pt-8 pb-4 border-b bg-muted/5">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2 font-black tracking-tighter text-2xl">
              <Package className="w-6 h-6 text-primary" /> Finish Order
            </SheetTitle>
          </SheetHeader>
        </div>
        ...
        <ScrollArea className="flex-grow px-6">
          <div className="py-6 space-y-8">
            <div className="space-y-4">
              ...
              <div className="grid gap-3">
                <div className="space-y-1">
                  ...
                  <Input placeholder="Enter your name" value={customerInfo.name} onChange={(e) => updateCustomerInfo({ name: e.target.value })} className="rounded-xl h-12" />
                </div>
                <div className="space-y-1">
                  <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground ml-1">Delivery Address</Label>
                  <Input placeholder="Flat/House, Building, Landmark" value={customerInfo.address} onChange={(e) => updateCustomerInfo({ address: e.target.value })} className="rounded-xl h-12" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-2">
                <CreditCard className="w-3.5 h-3.5" /> Payment Preference
              </h3>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Order Selection</h3>
                <Button variant="ghost" size="sm" onClick={clearCart} className="text-[9px] text-destructive uppercase font-black hover:bg-destructive/10">Empty Bag</Button>
              </div>
              <div className="space-y-3">
                {cart.map((item) => (
                  <div key={`${item.id}-${item.variant}`} className="flex gap-4 bg-muted/20 p-4 rounded-2xl border border-primary/5 group transition-all hover:bg-white hover:shadow-sm">
                    <div className="flex-grow space-y-1">
                      <h4 className="font-bold text-sm tracking-tight uppercase">{item.name} {item.variant && <span className="text-[9px] text-muted-foreground ml-1">({item.variant})</span>}</h4>
        ...
        <div className="p-6 bg-white border-t space-y-4 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.1)]">
          ...
          <Button onClick={handleWhatsAppOrder} disabled={isProcessing} className="w-full h-16 bg-primary text-lg font-black rounded-xl shadow-xl shadow-primary/20 transition-all active:scale-[0.98]">
            {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Order on WhatsApp <Send className="ml-2 w-4 h-4" /></>}
          </Button>
          <p className="text-[9px] text-center text-muted-foreground font-medium uppercase tracking-widest">Verified Pure Veg Kitchen</p>
        </div>
      </SheetContent>
    </Sheet>
  );
